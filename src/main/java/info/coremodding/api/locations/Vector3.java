...
public class Vector3 implements Cloneable
{
    ...
    {
        return world.setBlock(this.intX(), this.intY(), this.intZ(), block,
                metadata, notify);
    }
    ...
    public ForgeDirection toForgeDirection()
    {
        for (ForgeDirection direction : ForgeDirection.VALID_DIRECTIONS)
        {
            if (this.x == direction.offsetX && this.y == direction.offsetY
                    && this.z == direction.offsetZ) { return direction; }
        }
    ...
    {
        return new Vector3(Math.max(x, other.x), Math.max(y, other.y),
                Math.max(z, other.z));
    }
    
    public Vector3 min(Vector3 other)
    {
        return new Vector3(Math.min(x, other.x), Math.min(y, other.y),
                Math.min(z, other.z));
    }
    ...
    {
        if (this.z == 0.0F) { return this.zCrossProduct(); }
     ...
     * messed up. Y and Z are flipped. Yaw is displaced by 90 degrees. Pitch is
     * inversed.
     * 
     * @param rotationYaw
     * @param rotationPitch
    ...
    {
        Vector3 lookVector = Vector3.getDeltaPositionFromRotation(rotationYaw,
                rotationPitch);
        Vector3 reachPoint = this.clone().translate(
                lookVector.clone().scale(reachDistance));
            ...
            boolean collisionFlag)
    {
        MovingObjectPosition pickedBlock = this.rayTraceBlocks(world,
                reachPoint.clone(), collisionFlag);
    ...
    public MovingObjectPosition rayTrace(World world, boolean collisionFlag,
    ...
    public MovingObjectPosition rayTraceBlocks(World world, Vector3 vec,
            boolean collisionFlag)
    {
    ...
    public MovingObjectPosition rayTraceEntities(World world, Vector3 target)
    {
        ...
        List<Entity> entitiesInBounds = world
                .getEntitiesWithinAABBExcludingEntity(null, boxToScan);
        double closestEntity = reachDistance;
        
        if (entitiesInBounds == null || entitiesInBounds.isEmpty()) { return null; }
        for (Entity possibleHits : entitiesInBounds)
        {
                    ...
                    && possibleHits.boundingBox != null)
            {
                ...
                AxisAlignedBB aabb = possibleHits.boundingBox.expand(border,
                        border, border);
                MovingObjectPosition hitMOP = aabb.calculateIntercept(
                        startingPosition, target.toVec3());
                ...
                if (hitMOP != null)
                {
                    if (aabb.isVecInside(startingPosition))
                    {
                        ...
                        {
                            pickedEntity = new MovingObjectPosition(
                                    possibleHits);
